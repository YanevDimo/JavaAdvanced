...
public class P08HandsOfCards {
    public static void main(String[] args) {
        ...
        while (!input.equals("JOKER")) {
            ...
            if (!playersCards.containsKey(name)) {
                ...
                Set<String> currentCard = playersCards.get(name);
                currentCard.addAll(cards);
                playersCards.put(name, currentCard);
            }
    ...
    private static int getCardPoints(Set<String> cards) {
        ...
        for (String card : cards) {
            int points = 0;
            if (card.contains("10")) {
                char streigth = card.charAt(2);
                points = 10 * pointsValues.get(streigth);
            } else {
                char number = card.charAt(0);
                char streigth = card.charAt(1);
                points = pointsValues.get(streigth) * pointsValues.get(number);
            }
            sum += points;
        }
        return sum;
